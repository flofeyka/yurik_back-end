...
export class AgreementService {
  ...
  async getAgreements(userId: number, type: | "В работе"
    ...
    return agreements.map(
      (agreement: Agreement): AgreementsListDto => new AgreementsListDto(agreement)
    );
  }
  ...
  async editAgreement(
    ...
    if (editDealDto.dealText?.text) {
      ...
      if (editDealDto.dealText.generate) {
        const message: any = await this.gigachatService.sendMessage({
          ...
          messages: [
            ...
            Какие данные я могу предоставить ${agreement.members.map((member: AgreementMember): string => {
              ...
              `;
                        ...
                        Все этапы договора: ${agreement.steps.map((step: AgreementStep): string => {
      ...
      agreement.pdf = await this.pdfService.convertMarkdownToPdf(agreement.text, user);
    }
  ...
  async completeAgreement(agreement: Agreement, userId: number): Promise<AgreementDto> {
    ...
    if (agreement.steps.find((step: AgreementStep): boolean => step.status === "В процессе" || step.status == "Ожидает")) {
      throw new BadRequestException("Не все шаги завершены.");
    }

    if (agreement.steps.every((step: AgreementStep): boolean => step.status === "Отклонён")) {
  ...
  async rejectAgreement(agreement: Agreement, userId: number): Promise<AgreementDto> {
    ...
    if (!agreement.steps.find((step: AgreementStep) => step.status === "Отклонён")) {
  ...
  async addAgreementPhotos(agreement: Agreement, images: string[], userId: number): Promise<AgreementDto> {
    ...
    const imagesFound: AgreementImage[] = await Promise.all(
      images.map(async (image: string): Promise<AgreementImage> => {
        if (agreement.images.find((i: AgreementImage): boolean => image === i.image.name)) {
        ...
        const agreementImage: InsertResult = await this.agreementImageRepository.createQueryBuilder().insert().into(AgreementImage).values({
          agreement: agreement,
          image: imageFound
        }).execute();
