...
export class StepService {
  constructor(
    ...
    @InjectRepository(StepImage)
    private readonly stepImageRepository: Repository<StepImage>,
  ...
  async getStep(
    ...
    if (
      !step ||
      !agreement.steps.find((step: AgreementStep) => step.id === stepId)
    ) {
  ...
  async changeStepsOrder(
    agreement: Agreement,
    stepsDto: ChangeOrder,
    userId: number,
  ): Promise<AgreementDto> {
    if (agreement.status !== 'Черновик') {
      throw new BadRequestException(
        'Вы не можете изменить порядок этапов, так как договор был подписан.',
      );
    }
    if (agreement.initiator.user.id !== userId) {
      throw new BadRequestException(
        'Вы не можете изменить порядок этапов в договоре, так как не являетесь его инициатором.',
      );
    }
    if (stepsDto.steps.length !== agreement.steps.length) {
      throw new BadRequestException(
        'Количество этапов в запросе не совпадает с количеством этапов в договоре.',
      );
    }
    agreement.steps = await Promise.all(
      stepsDto.steps.map(async (step: { id: UUID }, i: number) => {
        ...
        if (
          !agreement.steps.find(
            (agreementStep: AgreementStep) => agreementStep.id === stepFound.id,
          )
        ) {
          throw new BadRequestException(`Этап с id ${step.id} не найден.`);
        }
  ...
  async takeStep(
    ...
    if (agreement.status !== 'Активный') {
      throw new BadRequestException(
        'Нельзя взять этап в недействующем договоре',
      );
    }
    if (step.status !== 'Ожидает') {
      throw new BadRequestException(
        'Нельзя взять этап, так как он не находится в статусе ожидания',
      );
    }
  ...
  async cancelStep(step: AgreementStep): Promise<AgreementStepDto> {
    if (step.status !== 'Ожидает' && step.status !== 'В процессе') {
      throw new BadRequestException(
        'Нельзя отменить этап, так как он не находится в статусе ожидания или в процессе',
      );
    }
  ...
  async completeStep(
    ...
    if (
      ...
      throw new BadRequestException(
        'Нельзя завершить этап, так как он не находится в статусе ожидания или в процессе',
      );
    }
  ...
  async deleteStep(
    ...
    if (agreement.status !== 'Черновик') {
      throw new BadRequestException(
        'Нельзя удалить этап, так как договор не является черновиком',
      );
    }
    ...
    if (!agreement.steps.find((step: AgreementStep) => step.id === stepId)) {
      throw new BadRequestException(
        'Нельзя удалить этап, так как он не принадлежит данному договору',
      );
    }
    const deleteResult: DeleteResult = await this.stepRepository.delete({
      id: step.id,
    });
  ...
  async editStep(
    ...
    if (
      step.start &&
      step.end &&
      !(new Date(editStepDto.start) < new Date(editStepDto.end))
    ) {
      throw new BadRequestException(
        'Дата начала этапа не может быть позже даты окончания этапа',
      );
    }
    const member: AgreementMember = this.memberService.findMember(
      agreement,
    ...
    if (editStepDto?.images && editStepDto.images.length > 0) {
      const imagesAdded: PromiseSettledResult<StepImage>[] =
        await Promise.allSettled(
          editStepDto.images.map(async (image: string): Promise<StepImage> => {
      ...
      const successfulImages: StepImage[] = imagesAdded
        .filter(
          (
            result: PromiseSettledResult<StepImage>,
          ): result is PromiseFulfilledResult<StepImage> =>
            result.status === 'fulfilled',
        )
        .map(
          (result: PromiseFulfilledResult<StepImage>): StepImage =>
            result.value,
        );
  ...
  async addStep(step: Step, agreement: Agreement): Promise<AgreementStepDto> {
    ...
    if (!member || member.agreement.id !== agreement.id) {
      throw new NotFoundException(
        'Пользователь не был найден в списке участников договора.',
      );
    }
      ...
      .values([
        {
          ...
          order:
            agreement.steps.length > 1
              ? agreement.steps.reduce(function (prev, current) {
    ...
    if (step.images?.length > 0) {
      const imagesAdded = await Promise.allSettled(
        step.images.map(async (image: string) => {
          const imageSaved: Image =
            await this.imagesService.getImageByName(image);
      ...
      const successfulImages: StepImage[] = imagesAdded
        .filter(
          (result): result is PromiseFulfilledResult<StepImage> =>
            result.status === 'fulfilled',
        )
