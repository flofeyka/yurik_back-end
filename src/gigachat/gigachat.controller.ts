...
export class GigachatController {
    ...
    @Post('/send')
    async sendMessage(@Body() messageDto: SendMessageDto, @Req() request: Request) {
        return this.gigachatService.sendToGigaChat(messageDto.message, request.cookies.gigachat_accessToken)
    }
}
